...
- 최종 R 배치까지 닫힌 정상 상태를 활성 배치 누락으로 거부하던 감사 하네스를
  수정하고, 최종 종결에는 모든 선행 배치가 닫혀야 함을 fail-closed로 검증합니다.
- 구형 SQLite schema에서 신규 zero-count readback 필드가 추가될 때 정상
  업그레이드를 데이터 불일치로 오판하던 검증기를 기존 핵심 row 보존 비교로
  수정했습니다.
- 한 행에 여러 설정 필드가 있을 때 옆 칸의 설명을 공유하던 추출기 오류를
  수정했습니다. 접근가능·접근차단 IP 등의 서로 다른 안내를 구분합니다.
