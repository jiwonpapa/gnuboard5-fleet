...
실제 재이관은 [`도메인별 재사용 이관 배치`](docs/roadmap/DOMAIN_REUSE_MIGRATION_PLAN.md)의 R00 → R36 순서로 진행합니다. 과거 B00 → B10은 서버 기반 작업 이력일 뿐 전체 전환 완료 근거가 아닙니다. R03까지 공통 기반, 설치·인증·보안, 사이트·활동·backup 172개 항목을 닫았지만 legacy·Core·필수 capability 540개가 남아 있으므로 전체 판정은 여전히 `MIGRATION_STATIC FAIL`입니다. 현재 활성 배치는 R04 SSH·SFTP·terminal입니다.
