...
use Api\Auth\External\Provider\Support\GoogleExternalAuthResultMapper;
...
final readonly class GoogleExternalAuthProviderAdapter implements ExternalAuthProviderAdapter
{
    /** @var list<string> */
    private const SUPPORTED_FLOWS = ['login', 'account_link'];

    /** @var list<string> */
    private const DEFAULT_SCOPES = ['openid', 'email', 'profile'];
    ...
    public function describe(): array
    {
        return [
            ...
            'description' => 'Google OAuth 2.0 / OpenID Connect 기반 외부 로그인 공급자입니다. `openid email profile` 기준으로 사용자 정보를 정규화합니다.',
    ...
    public function start(array $request): array
    {
        ...
        return [
            ...
            'authorization_url' => $this->authorizeEndpoint() . '?' . $query,
            'callback_method' => 'GET',
            'provider_meta' => [
                'scopes' => $scopes,
    ...
    public function complete(array $request): array
    {
        ...
        $resultMapper = new GoogleExternalAuthResultMapper($this->providerMode());
        ...
        if ($code === '') {
            throw ApiException::badRequest('google authorization code가 필요합니다.');
        }

        $claims = is_array($request['claims'] ?? null) ? $request['claims'] : [];
        $callbackUrl = trim((string)($claims['callback_url'] ?? ''));
        if ($callbackUrl === '') {
            throw ApiException::badRequest('google callback_url이 request_token에 없습니다.');
        }

        $tokenResponse = $this->httpClient->postForm($this->tokenEndpoint(), [
            'code' => $code,
            'client_id' => $this->clientId(),
    ...
    private function assertConfigured(): void
    {
        ...
        throw ApiException::forbidden('google 외부 인증 설정이 완료되지 않았습니다.');
    }
    ...
    {
        return $this->providerConfig->providerString('google', 'authorize_url', $this->endpointCatalog->endpoint('google', 'authorize'));
    }

    private function tokenEndpoint(): string
    {
        return $this->providerConfig->providerString('google', 'token_url', $this->endpointCatalog->endpoint('google', 'token'));
    }

    private function userinfoEndpoint(): string
    {
        return $this->providerConfig->providerString('google', 'userinfo_url', $this->endpointCatalog->endpoint('google', 'userinfo'));
    }

    private function clientId(): string
    {
        return $this->providerConfig->providerString('google', 'client_id', (string)$this->g5Config->get('cf_google_clientid', ''));
    }

    private function clientSecret(): string
    {
        return $this->providerConfig->providerString('google', 'client_secret', (string)$this->g5Config->get('cf_google_secret', ''));
    }

    private function isEnabled(): bool
    {
        return $this->providerConfig->providerEnabled('google', true);
    }
    ...
    private function normalizeFlow(string $flow): string
    {
        ...
        if (!in_array($normalized, self::SUPPORTED_FLOWS, true)) {
            throw ApiException::badRequest('google provider는 login, account_link flow만 지원합니다.');
        }
