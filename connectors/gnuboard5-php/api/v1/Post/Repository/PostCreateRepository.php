...
final class PostCreateRepository extends PostWriteSupport
{
...
SET
    wr_num = (SELECT IFNULL(MIN(wr_num) - 1, -1) FROM {$writeTable} as sq),
    wr_reply = '',
    wr_parent = 0,
    wr_is_comment = 0,
    wr_comment = 0,
...
SQL;
        ...
        $this->executeStatement(
            $sql,
            [
                'ca_name' => (string)$category,
                'wr_option' => (string)$option,
                'wr_subject' => $subject,
                'wr_content' => $content,
                'wr_link1' => trim((string)$link1),
                'wr_link2' => trim((string)$link2),
                'mb_id' => $memberId,
                'wr_name' => $memberName,
        ...
        if ($postId <= 0) {
            $postId = $this->resolveInsertedWriteId($writeTable, $memberId, $now, $subject, $content, '');
        }
        if ($postId <= 0) {
            throw ApiException::serverError('게시글 생성에 실패했습니다.');
        }
        ...
        if ($isNotice) {
            $this->noticeRepository->setNotice($boTable, $postIdSafe, true);
        }
